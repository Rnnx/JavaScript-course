...
events that happened during the game. The values are the events themselves, and the keys 
are the minutes in which each event happened (a football game has 90 minutes plus some 
extra time).
...
console.log(
  `An event happened, on average, every ${Math.trunc(
    Array.from(gameEvents.keys()).pop() / gameEvents.size
  )} minutes.`
);
